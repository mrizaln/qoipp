...
namespace qoipp
{
     ...
     */
    Result<std::size_t> encode_into(Span out_buf, CSpan in_data, Desc desc);
     ...
     * - `Error::NotEnoughSpace` if the `out` buffer doesn't have enough space for encoded data.
     */
    Result<std::size_t> encode_into(Span out_buf, PixelGenFun in_func, Desc desc);
     ...
     * - `Error::NotEnoughSpace` if the `out` buffer doesn't have enough space for encoded data.
     */
    Result<std::size_t> encode_into(ByteSinkFun out_func, CSpan in_data, Desc desc);
